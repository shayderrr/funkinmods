...
function onCreate()
    ...
    doTweenAlpha('thefunny','camHUD',0,0.1,'linear')
    debugPrint('funny')
end

function onBeatHit()
    --debugPrint('the')
    if curBeat ==  128 then--128
    ...
    if curBeat == 32 then -- 32
        triggerEvent('Add Camera Zoom',0.2,0.2)
        doTweenAlpha('thefunny','camHUD',1,crochet * 0.01,'quintOut')
    end 
    if curBeat > 32 and curBeat <= 60  then
