...
void main() {
	...
	float xpart = openfl_TextureCoordv.x;
	
	float xfuel = pow(1.0-abs(2.0*xpart-1.0),0.8)/zoom;
	
	float timeSpeed = 0.5*speed;
	...
	float sparkLife = min((10.0*strength)*(1.0-min((sparkGridIndex.y+(190.0*realTime/sparkGridSize))/(24.0-20.0*sparkRandom),1.0)),(1.0*strength));
	...
	gl_FragColor = vec4(sparks*2.0,brightness(sparks));
}